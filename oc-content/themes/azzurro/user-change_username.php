<?php
    ...
    function custom_meta_title($data){
        return __('Change username', 'azzurro');;
    }
    osc_current_web_theme_path('header.php') ;
    $osc_user = osc_user();
?>
<h1><?php _e('Change username', 'azzurro'); ?></h1>
...
$(document).ready(function() {
    $('form#change-username').validate({
        ...
        messages: {
            s_username: {
                required: '<?php echo osc_esc_js(__("Username: this field is required", "azzurro")); ?>.'
            }
        },
            ...
            cInterval = setInterval(function(){
                $.getJSON(
                    ...
                    function(data){
                        clearInterval(cInterval);
                        if(data.exists==0) {
                            $("#available").text('<?php echo osc_esc_js(__("The username is available", "azzurro")); ?>');
                        } else {
                            $("#available").text('<?php echo osc_esc_js(__("The username is NOT available", "azzurro")); ?>');
                        }
                    }
                );
            }, 1000);
    ...
    <div class="resp-wrapper">
        ...
        <form action="<?php echo osc_base_url(true); ?>" method="post" id="change-username">
            ...
            <div class="control-group">
                <label class="control-label" for="s_username"><?php _e('Username', 'azzurro'); ?></label>
                <div class="controls">
